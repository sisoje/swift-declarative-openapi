...
extension SpecGenerator {
    ...
    func renderResponses(_ operations: [Operation]) -> String {
        ...
        output += "    }\n\n"
        output += "    /// Thrown when the transport yields something other than HTTP —\n"
        output += "    /// same rule, nothing lost.\n"
        output += "    struct NonHTTPResponse: Error {\n"
        output += "        let operation: Operation\n"
        output += "        let data: Data\n"
        ...
        output += "        ) throws -> Data {\n"
        output += "            guard let http = output.response as? HTTPURLResponse else {\n"
        output += "                throw NonHTTPResponse(operation: operation, data: output.data, response: output.response)\n"
        output += "            }\n"
        output += "            let declared = successStatuses(operation)\n"
        output += "            let status = http.statusCode\n"
        output += "            let expected = declared.isEmpty ? (200 ..< 300).contains(status) : declared.contains(status)\n"
        output += "            guard expected else {\n"
        output += "                throw ResponseError(operation: operation, status: status, data: output.data, response: http)\n"
        output += "            }\n"
