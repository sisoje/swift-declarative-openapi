...
- `security:` declarations generate the README's Open Spec auth concept per case: `var securitySchemes: Set<String>` (operation-level overrides the document default; `security: []` marks an operation public; OR-alternatives are flattened) and `var needsAuth: Bool`. Omitted entirely when the spec declares no security. Wiring layers gate on the scheme names — see `SupabaseAuthWiring.swift`, where `authorized(accessToken:)` applies the bearer only where the spec requires `UserAuth` and a required-but-missing token throws `MissingAccessToken` at `.request`.
